...
*/
class T_vat_reg_dtl_ppj_non_pln_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','t_vat_reg_dtl_ppj_npl_id');
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('transaksi/t_vat_reg_dtl_ppj_non_pln');
            $table = $ci->t_vat_reg_dtl_ppj_non_pln;
            ...
            $data['rows'] = $table->getAll(0, -1, 't_vat_reg_dtl_ppj_npl_id', 'asc');
            $data['success'] = true;
