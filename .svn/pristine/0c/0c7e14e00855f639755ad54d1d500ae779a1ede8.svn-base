...
/**
* Json library
* @class t_rep_lap_bpps_terakhir_bayar_controller
...
class T_rep_lap_bpps_terakhir_bayar_controller {
    ...
    function readData(){
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('pelaporan/t_rep_lap_bpps_terakhir_bayar');
            $table = $ci->t_rep_lap_bpps_terakhir_bayar;

            $result = $table->getData($p_vat_type_id);
    ...
    function readHTML(){
        ...
        $data = array('rows' => "", 'page' => 1, 'records' => 0, 'total' => 1, 'success' => false, 'message' => '');
        ...
        try {
            ...
            $html = '';
            $result = $table->getData($p_vat_type_id);
            $html = $html . '<table border="1" cellspacing="0" cellpadding="3px" width="100%"><tr>';


            $html = $html . '<th>NO</th>';
            $html = $html . '<th>NO AYAT</th>';
            $html = $html . '<th>NO KOHIR</th>';
            $html = $html . '<th>NAMA WP</th>';
            $html = $html . '<th>ALAMAT</th>';
            $html = $html . '<th>NPWPD</th>';
            $html = $html . '<th>PEMBAYARAN <br>TERAKHIR</th>';
            $html = $html . '<th>TGL BAYAR <br>TERAKHIR</th>';
            $html = $html . '<th width=140>MASA PAJAK</th>';
            $html = $html . '</tr>';
            ...
            foreach($result as $item2) {
                if ($j==0){
                    ...
                    if ($item['npwpd'] == $item2['npwpd']){
                        ...
                        $html = $html . '<tr>';
                        $html = $html . '<td align="center">'.($i+1).'</td>';
                        $html = $html . '<td align="center">'.$item["kode_jns_pajak"]." ".$item["kode_ayat"].'</td>';
                        //$html = $html . '<td align="center">'.$item["nama_ayat"].'</td>';
                        $html = $html . '<td align="left">'.$item['no_kohir'].'</td>';
                        $html = $html . '<td align="left">'.$item['wp_name'].'</td>';
                        $html = $html . '<td align="left">'.$item['address'].'</td>';
                        $html = $html . '<td align="left">'.$item['npwpd'].'</td>';
                        $html = $html . '<td align="right">'.number_format($item["jumlah_terima"], 2, ',', '.').'</td>';
                        $html = $html . '<td align="left">'.date("d-m-Y", strtotime($item['payment_date'])).'</td>';
                        $html = $html . '<td align="left">'.$item['masa_pajak'].'</td>';
                        $html = $html . '</tr>';
                        $item=$item2;
                        $i=$i+1;
                    }           
    ...
    function excel(){
        ...
        try {
            ...
            startExcel(date("dmy") . '_rep_lap_bpps_terakhir_bayar.xls');
            echo '<html>';
    ...
    function excelRekap(){
        ...
        try {
            ...
            startExcel(date("dmy") . '_rep_lap_bpps_terakhir_bayar_REKAP.xls');
