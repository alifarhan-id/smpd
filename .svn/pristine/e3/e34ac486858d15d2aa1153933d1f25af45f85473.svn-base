...
$("#tab-3").on("click", function(event) {
    ...
    var grid = $('#grid-table');
    p_app_user_id = grid.jqGrid ('getGridParam', 'selrow');
    app_user_name = grid.jqGrid ('getCell', p_app_user_id, 'app_user_name');

    if(p_app_user_id == null) {
...
<script>
    jQuery(function($) {
        ...
        jQuery("#grid-table").jqGrid({
            url: '<?php echo WS_JQGRID."administration.p_app_user_controller/crud"; ?>',
            datatype: "json",
            mtype: "POST",
            colModel: [
                {label: 'ID', name: 'p_app_user_id', key: true, width: 5, sorttype: 'number', editable: true, hidden: true},
                {label: 'Username',name: 'app_user_name',width: 150, align: "left",editable: true,
                    editoptions: {
                        size: 30,
                        maxlength:32
                    },
                ...
                {label: 'Email',name: 'email_address',width: 150, hidden:false, align: "left",editable: true,
                    edittype: 'text',
                    editoptions: {
                        size: 30,
                        maxlength:60
                ...
                {label: 'Password',name: 'user_pwd',width: 150, hidden:true, align: "left",editable: true,
                    edittype: 'password',
                    editoptions: {
                        size: 30,
                        maxlength:15,
                ...
                {label: 'Status Aktif',name: 'p_user_status_id',width: 120, align: "left",editable: true, edittype: 'select', hidden:true,
            ...
            onSelectRow: function (rowid) {
                ...
                var celValue = $('#grid-table').jqGrid('getCell', rowid, 'p_app_user_id');
                var celCode = $('#grid-table').jqGrid('getCell', rowid, 'app_user_name');

                var grid_detail = jQuery("#grid-table-detail");
                if (rowid != null) {
                    grid_detail.jqGrid('setGridParam', {
                        url: "<?php echo WS_JQGRID."administration.p_app_user_role_controller/crud"; ?>",
                        postData: {p_app_user_id: celValue}
                    });
                    var strCaption = 'Mapping Role :: ' + celCode;
                    grid_detail.jqGrid('setCaption', strCaption);
                    $("#temp_p_app_user_id").val(celValue);
        ...
        jQuery('#grid-table').jqGrid('navGrid', '#grid-pager',
...

            {
                ...
                beforeShowForm: function (e, form) {
                    ...
                    $("#app_user_name").prop("readonly", false);
        ...
        jQuery("#grid-table-detail").jqGrid({
            ...
            colModel: [
                ...
                {label: 'User ID', name: 'p_app_user_id', width: 5, sorttype: 'number', editable: true, hidden: true},
                {label: 'Role', name: 'p_app_role_id', width: 250, align: "left", editable: true, hidden:true,
                    editrules: {edithidden: true, required:true},
                    edittype: 'select',
                    editoptions: {
                        dataUrl: "<?php echo WS_JQGRID.'administration.p_app_user_role_controller/html_select_options_p_app_role'; ?>",
                        dataInit: function(elem) {
                            $(elem).width(250);  // set the width which you need
                        },
                        postData : {
                            p_app_user_id : function() {
                                var selRowId =  $("#grid-table").jqGrid ('getGridParam', 'selrow');
                                var p_app_user_id = $("#grid-table").jqGrid('getCell', selRowId, 'p_app_user_id');

                                return p_app_user_id;
                            },
                            p_app_role_id : function(){
                                var selRowId =  $("#grid-table-detail").jqGrid ('getGridParam', 'selrow');
                                var p_app_role_id = $("#grid-table-detail").jqGrid('getCell', selRowId, 'p_app_role_id');
                ...
                {label: 'Role Name', name: 'code', width: 120, align: "left", editable: false}
            ],
            height: '100%',
            //autowidth: false,
            width:500,
            ...
            //memanggil controller jqgrid yang ada di controller crud
            editurl: "<?php echo WS_JQGRID.'administration.p_app_user_role_controller/crud'; ?>",
