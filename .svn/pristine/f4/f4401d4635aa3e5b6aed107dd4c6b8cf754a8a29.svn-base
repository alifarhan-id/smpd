...
$("#tab-1").on("click", function(event) {
    ...
    loadContentWithParams("administration.p_application",{});
});
</script>
...
<script>
    $(function($) {
        ...
        $("#grid-table").jqGrid({
            url: '<?php echo WS_JQGRID."administration.p_app_menu_controller/crud"; ?>',
            postData: { p_application_id : <?php echo $this->input->post('p_application_id'); ?>},
            datatype: "json",
            mtype: "POST",
            colModel: [
                {label: 'ID', name: 'p_app_menu_id', key: true, width: 5, sorttype: 'number', editable: true, hidden: true},
                {label: 'Parent ID', name: 'parent_id', width: 5, sorttype: 'number', editable: true, hidden: true},
                {label: 'Title',name: 'code',width: 150, align: "left",editable: true,
                    editoptions: {
                        size: 30,
                        maxlength:255
                    },
                ...
                {label: 'Url',name: 'file_name',width: 150, align: "left",editable: true,
                    editoptions: {
                        size: 30,
                        maxlength:255
                    }
                },
                {label: 'Order',name: 'listing_no',width: 150, align: "left",editable: true,
                    editoptions: {
                        size: 30,
                        maxlength:255,
            ...
            editurl: '<?php echo WS_JQGRID."administration.p_app_menu_controller/crud"; ?>',
            caption: "Child Menu"
            ...
            },
            {
                editData: {
                    ...
                    p_application_id: function() {
                        return <?php echo $this->input->post('p_application_id'); ?>;
                    }
                },
                //new record form
                serializeEditData: serializeJSON,
...
<script>
    function reloadTreeMenu() {
        var source =
        {
            ...
            id: 'id',
            url: '<?php echo WS_JQGRID."administration.p_app_menu_controller/tree_json?p_application_id=".$this->input->post("p_application_id")."&code=".$this->input->post("code"); ?>',
            async: false
        };
    ...
    $(function($) {
        ...
        $('#tree-menu').on('select', function (event) {
            ...
            $('#grid-table').jqGrid('setGridParam', {
                url: '<?php echo WS_JQGRID."administration.p_app_menu_controller/crud"; ?>',
                postData: {parent_id: item.id, p_application_id: <?php echo $this->input->post('p_application_id'); ?>}
            });
