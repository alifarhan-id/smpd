-- Function: f_bphtb_registration_ppat_new(character varying, character varying, character varying, character varying, character varying, numeric, numeric, numeric, character varying, character varying, character varying, character varying, character, character, character, character, character, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, character varying, character varying, numeric, character varying, numeric, numeric, character varying, numeric, numeric, numeric, numeric, numeric, character varying)

-- DROP FUNCTION f_bphtb_registration_ppat_new(character varying, character varying, character varying, character varying, character varying, numeric, numeric, numeric, character varying, character varying, character varying, character varying, character, character, character, character, character, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, character varying, character varying, numeric, character varying, numeric, numeric, character varying, numeric, numeric, numeric, numeric, numeric, character varying);

CREATE OR REPLACE FUNCTION sikp.f_bphtb_registration_ppat_new(
    ...
    IN t_ppat_id numeric,
    IN i_check_potongan character varying,
    IN land_area_real numeric,
    IN land_price_real numeric,
    IN building_area_real numeric,
    IN building_price_real numeric,
...
select last_value into o_t_bphtb_registration_id from sikp.t_bphtb_registration_id_seq;
		begin
				if i_check_potongan = 'Y' THEN
					 v_customer_order_id := generate_id('sikp','t_customer_order','t_customer_order_id');
					 select '00'||LPAD(order_seq.nextval,6,'0')into v_order_no from dual;

					 INSERT INTO t_customer_order(
									 t_customer_order_id, order_no, p_rqst_type_id, p_order_status_id,
									 order_date, creation_date, created_by, updated_date,
									 updated_by)
					 VALUES (v_customer_order_id, v_order_no, 17, 1,
									sysdate, sysdate, i_user, sysdate,
									i_user);

					INSERT INTO "sikp"."t_bphtb_exemption" (
				    ...
				    select f_first_submit_engine(509,v_customer_order_id, i_user) into v_submit_return from dual;
				end if;

				exception
              when others then
                  o_mess = 'Data potongan gagal di entry';
									delete from t_bphtb_registration where t_bphtb_registration_id = o_t_bphtb_registration_id;
									o_t_bphtb_registration_id := null;
                  Return;
		end;
