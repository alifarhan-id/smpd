...
*/
class P_vat_type_group_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','p_vat_type_group_id');
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('parameter/p_vat_type_group');
            $table = $ci->p_vat_type_group;
            ...
            if(!empty($p_vat_group_id)) {
                $req_param['where'][] = 'pr.p_vat_group_id = '.$p_vat_group_id;
            }
    ...
    function readLov() {
        ...
        $sort = getVarClean('sort','str','p_vat_type_group_id');
