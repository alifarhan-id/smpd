...
jQuery(function ($) {
    ...
    jQuery("#grid-table-history").jqGrid({
            url: '<?php echo WS_JQGRID . "pelaporan.t_laporan_denda_controller/readData"; ?>',
            datatype: "json",
            mtype: "POST",
            colModel: [
            ...
            caption: "LAPORAN DENDA TANGGAL TAP"
        });
});
...
jQuery(function ($) {
    ...
    jQuery("#grid-table-rekap").jqGrid({
        url: '<?php echo WS_JQGRID . "pelaporan.t_laporan_denda_controller/readRekap"; ?>',
        datatype: "json",
        mtype: "POST",
        colModel: [
            {label: 'No. ',name: 'nomor',width: 75, align: "left"},
        ...
        caption: "LAPORAN REKAP DENDA TANGGAL TAP"
...
function showData(){
    ...
    if (p_settlement_type_id==''||start_date==''||end_date==''||status_bayar==''){
        ...
        //var pager_selector = "#grid-pager-bpps2";
            ...
            $("#grid-table-history").jqGrid("setCaption", "LAPORAN DENDA TANGGAL TAP");
...
function showDataRekap(){
    ...
    if (p_settlement_type_id==''||start_date==''||end_date==''||status_bayar==''){
        ...
        //var pager_selector = "#grid-pager-bpps2";
            ...
            $("#grid-table-rekap").jqGrid("setCaption", "LAPORAN REKAP DENDA TANGGAL TAP");
...
<script type="text/javascript">
    ...
    function toExcel() {
        if ($('#form_vat_dtl_id').val()==''||$('#start_period').val()==''||$('#end_period').val()==''||$('#status').val()==''){
            swal('Informasi','Semua Harus Diisi','info');
            $ ("#gview_grid-table-history").hide();
            $ ("#gview_grid-table-rekap").hide();
            return;
        ...
        var url = "<?php echo WS_JQGRID . "pelaporan.t_laporan_denda_controller/excel/?"; ?>";
    ...
    function toExcelRekap() {
        ...
        var url = "<?php echo WS_JQGRID . "pelaporan.t_laporan_denda_controller/excelRekap/?"; ?>";
        url += "<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>";
        url += "&p_settlement_type_id=" +  $('#form_ketetapan_id').val();
        //tipe_ayat = $('#form_vat_dtl_id').val();
        url += "&start_date=" +  $('#start_period').val();
