<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span>PELAPORAN PAJAK MANUAL SKPDKB TAMBAHAN</span>
        </li>
    </ul> 
</div>
    ...
    <div class="col-md-12">
        <div class="portlet light bordered">
            <div class="form-body" id="grid-pager">
                ...
                <div class="space-2"></div>
                <div class="row">
                    <label class="control-label col-md-3">Mode Denda</label>
                    <div class="col-md-5">
                        <div class="input-group">
                            <select id="mode_denda" name="mode_denda" class="FormElement form-control" >
                                <option value="">--Pilih Mode Denda--</option>
                                <option value="1">+100% Denda</option>
                                <option value="2">Tanpa Denda</option>
                            </select>                 
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function save(){
        ...
        if (mode_denda == "" || mode_denda == 0 || mode_denda == false || mode_denda == undefined ||  mode_denda == null){
            swal('Informasi',"Mode Denda harus diisi",'info'); 
            return;
        }



        var var_url = "<?php echo WS_JQGRID . "transaksi.t_vat_setllement_manual_skpdkb_tambahan_controller/insertUpdate/?"; ?>";
        ...
        $.getJSON(var_url, function( items ) {
            ...
            if(items.rows.o_cust_order_id == 0||items.rows.o_cust_order_id == "0" || items.rows.o_cust_order_id == ""){
                ...
                swal('Informasi',items.rows.o_mess,'info'); 
                url = '<?php echo base_url(); ?>'+'cetak_formulir_skpdkb_tambahan/pageCetak?t_cust_order_id='+items.rows.o_cust_order_id;
                openInNewTab(url);
