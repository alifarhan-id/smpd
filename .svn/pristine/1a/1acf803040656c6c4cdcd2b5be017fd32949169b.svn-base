CREATE OR REPLACE FUNCTION sikp.f_vat_settlement_manual_wp(
    ...
    OUT o_pay_key character varying,
    OUT o_vat_set_id character varying)
  RETURNS record AS
$BODY$
declare
   ada int;
   ...
   ln_pct_class     Number(5,2);
     ...
     ls_payment_key VARCHAR;
		no_kohir VARCHAR;
		masa_pajak_temp VARCHAR;
		rec_i record;
		temp_function NUMERIC;
		jumlah_transaksi_harian numeric;
		jumlah_hari NUMERIC;
		counter_tanggal NUMERIC;
...
begin
   ...
   if i_finance_period_id is not null then
    ...
    if i_p_vat_type_dtl_id <> 11 then
			select i_p_vat_type_dtl_id= any('{15,17,21,27,30,41,42,43}'::int[]),i_p_vat_type_dtl_id= any('{15,17,21,27,30,41,42,43}'::int[]) into no_block,is_insidentil;
			...
			--cek transaksi harian
			select count(*) into jumlah_transaksi_harian
				from t_cust_acc_dtl_trans 
				where t_cust_account_id = i_cust_account_id
				and trans_date BETWEEN to_date (i_start_period,'dd-mm-yyyy') and to_date(i_end_period,'dd-mm-yyyy');

			jumlah_hari := date_part('day',age(to_date(i_end_period,'dd-mm-yyyy') , to_date (i_start_period,'dd-mm-yyyy')))+1;
			if jumlah_hari != jumlah_transaksi_harian then 
				o_mess := 'Laporan masa pajak anda ini tidak sesuai dengan Laporan Rekapitulasi Penerimaan Harian';
				o_cust_order_id := 0;
				return;
			ELSE
				counter_tanggal := 0;
				for rec_i in (select *
											...
											ORDER BY trans_date)
				loop
					if rec_i.trans_date != (to_date (i_start_period,'dd-mm-yyyy')+ counter_tanggal) THEN
						o_mess := 'Laporan masa pajak anda ini tidak sesuai dengan Laporan Rekapitulasi Penerimaan Harian ';
						o_cust_order_id := 0;
						return;
					end if;
					counter_tanggal := counter_tanggal + 1;
				end loop;
			end if;
       ...
       WHERE vat_s.t_cust_account_id = i_cust_account_id
        ...
        /*SELECT count(*)INTO ada
            ...
            if no_block=false then
                    ...
                    SELECT count(*),string_agg(code, ', ') into ada,masa_pajak_temp
                    ...
                    if ada > 0 THEN
                            --o_mess := 'Maaf, masih terdapat ' || ada || ' periode pembayaran yang belum diselesaikan';
														o_mess := 'Pelaporan Anda tidak dapat diterima. Silahkan terlebih dahulu melakukan pembayaran masa pajak '||masa_pajak_temp;
														o_cust_order_id := -12;
														return;
                    END IF;

										SELECT count(*),string_agg(code, ', ') into ada3,masa_pajak_temp
										from (select * from p_finance_period 
										where p_finance_period_id not in (
																						 SELECT b.p_finance_period_id
																						 FROM p_finance_period b 
																						 left join t_vat_setllement a on a.p_finance_period_id = b.p_finance_period_id
																						 where a.t_cust_account_id = i_cust_account_id
																										and (a.t_vat_setllement_id is not null)
																						 )
										and end_date < (select start_date from p_finance_period where p_finance_period_id = i_finance_period_id)
										and start_date >= '01-01-2013'
										and start_date >= ldt_active_date_acc
										ORDER BY start_date);

                    if ada3 > 0 THEN
														o_mess := 'Pelaporan Anda tidak dapat diterima. Silahkan terlebih dahulu melaporkan pelaporan untuk bulan '||masa_pajak_temp;
														o_cust_order_id := 0;
														return;
                    END IF;
            end if;
    ...
    ELSE
       ...
       INSERT INTO t_customer_order(
         ...
         --ls_payment_key := f_get_payment_key_pjdl(i_user, ls_vat_type_code);
				select f_generate_kohir(v_customer_order_id) into no_kohir ;
       INSERT INTO t_vat_setllement(
            ...
            VALUES (id_vat, v_customer_order_id, sysdate, i_finance_period_id,
                ...
                i_trans_amount, i_vat_amount, ln_settlement_type_id , ls_is_settled,v_order_no,i_p_vat_type_dtl_id,null,null,no_kohir
						);
       ...
       if trunc(sysdate) > trunc(ldt_due_date ) then
          ...
          */
              ...
              ln_penalty_amt := round(ln_pct * nvl(i_vat_amount,0));
              --insert t_penalty
			 ...
			 --insert vat_set_dtl
			 begin
					select 1 into temp_function from f_gen_vat_dtl_trans(id_vat, i_user);
		 ...
		 if (v_vat_amount = 0 and i_p_vat_type_dtl_id != 11) then	
       ...
       o_mess := 'Laporan Anda sudah terkirim dan sedang dalam tahap verifikasi. Untuk mengetahui nomor bayar, mohon periksa Histori Transaksi secara berkala.';   
       o_cust_order_id := v_customer_order_id;
			 o_pay_key := ls_payment_key;
			 o_vat_set_id := id_vat;
    END IF;
