...
*/
class T_vat_reg_dtl_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','t_license_letter_id');
        $sord = getVarClean('sord','str','desc');

        $data = array('rows' => array(), 'page' => 1, 'records' => 0, 'total' => 1, 'success' => false, 'message' => '');

        $t_license_letter_id = getVarClean('t_license_letter_id','int',0);
        $t_vat_registration_id = getVarClean('t_vat_registration_id','int',0);

        try {

            $ci = & get_instance();
            $ci->load->model('transaksi/t_vat_reg_dtl');
            $table = $ci->t_vat_reg_dtl;
            ...
            $data['rows'] = $table->getAll(0, -1, 't_license_letter_id', 'asc');
