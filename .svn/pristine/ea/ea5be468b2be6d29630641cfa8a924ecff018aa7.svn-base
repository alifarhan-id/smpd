...
class Cetak_formulir_skpd_nihil extends CI_Controller{
	...
	function pageCetak() {
		...
		$pdf->Cell($lheader3, $this->height, "SKPDN", "R", 0, 'C');
		...
		*/
		...
		// $this->tulis("4. Jumlah kekurangan pembayaran Pokok Pajak (2-3d)", "L", $pdf);
		$pdf->Cell(5, $this->height, "", "L", 0, 'L');
		...
		$this->tulis("2. Apabila SKPDN ini tidak atau kurang dibayar setelah lewat waktu paling lama 15 hari kalender sejak SKPDN ini diterbitkan dikenakan", "L", $pdf);
		...
		$pdf->Cell($lbody1, $this->height, "No. SKPDN : ".$data["order_no"], "TR", 0, 'L');
		$pdf->Ln();
