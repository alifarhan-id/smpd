...
*/
class T_vat_reg_dtl_hotel_srvc_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','t_vat_reg_dtl_hotel_srvc_id');
        $sord = getVarClean('sord','str','desc');

        $data = array('rows' => array(), 'page' => 1, 'records' => 0, 'total' => 1, 'success' => false, 'message' => '');

        $t_vat_reg_dtl_hotel_srvc_id = getVarClean('t_vat_reg_dtl_hotel_srvc_id','int',0);
        $t_vat_registration_id = getVarClean('t_vat_registration_id','int',0);

        try {

            $ci = & get_instance();
            $ci->load->model('transaksi/t_vat_reg_dtl_hotel_srvc');
            $table = $ci->t_vat_reg_dtl_hotel_srvc;
            ...
            $data['rows'] = $table->getAll(0, -1, 't_vat_reg_dtl_hotel_srvc_id', 'asc');
            $data['success'] = true;
