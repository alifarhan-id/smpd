...
/**
* Json library
* @class t_history_skpdkb_controller
...
class T_history_skpdkb_controller {
    ...
    function readData(){
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('pelaporan/t_history_skpdkb');
            $table = $ci->t_history_skpdkb;

            $result = $table->getData($p_finance_period_id);

            for ($i = 0; $i < count($result); $i++) {
                $result[$i]['nomor'] = $i+1; 
                    if ($result[$i]['payment_date']=='') {
                        $result[$i]['status']='<td align="left" >Belum Bayar</td>';
                    }else{
                        $result[$i]['status']='<td align="left" >Sudah Bayar</td>';
                    }
            }
    ...
    function excel(){
        ...
        try {
            ...
            startExcel(date("dmy") . '_history_skpdkb.xls');
            echo '<html>';
    ...
    function excelRekap(){
        ...
        try {
            ...
            startExcel(date("dmy") . '_history_skpdkb_REKAP.xls');
