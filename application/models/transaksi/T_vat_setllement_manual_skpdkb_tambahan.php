 ...
 *
 */
class T_vat_setllement_manual_skpdkb_tambahan extends Abstract_model {
    ...
    function insertUpdate($t_cust_account_id,$p_finance_period_id,$npwd,$start_date,$end_date,$qty_room_sold,$trans_amount,$p_vat_type_dtl_id,$p_vat_type_dtl_cls_id,$mode_denda ){


        $ci =& get_instance();
        $userdata = $ci->session->userdata;

        $sql = "SELECT * FROM f_vat_settlement_manual_skpdkb_tambahan(?,?,?,?,?,?,?,?,?,?,?)";

        $query = $this->db->query($sql, array($t_cust_account_id,$p_finance_period_id,$npwd,$start_date,$end_date,$qty_room_sold,$trans_amount,$p_vat_type_dtl_id,$p_vat_type_dtl_cls_id,$userdata['app_user_name'],$mode_denda));
