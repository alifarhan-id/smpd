...
*/
class T_ppat_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','a.t_ppat_id');
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('transaksi/t_ppat');
            $table = $ci->t_ppat;
    ...
    function readDataCombo(){
        ...
        try {
            ...
            $html = "";
            $html.="<select name='personal_identification_id' id='personal_identification_id' class='form-control required' required>";
            $html.="<option value='' >Select Value</option>";
            foreach ($items as $item) {
              $html .=" <option value='" . $item['p_simple_parameter_list_id'] . "'>" . $item['code'] . "</option>";
            }
            $html .= "</select>";
