...
*/
class P_vat_group_controller {

    function read() {

        $page = getVarClean('page','int',1);
        $limit = getVarClean('rows','int',5);
        $sidx = getVarClean('sidx','str','p_vat_group_id');
        ...
        try {
            ...
            $ci = & get_instance();
            $ci->load->model('parameter/p_vat_group');
            $table = $ci->p_vat_group;
    ...
    function readLov() {
        ...
        $sort = getVarClean('sort','str','p_vat_group_id');
        ...
        try {
            ...
            if(!empty($searchPhrase)) {
                $table->setCriteria("upper(group_code) like upper('%".$searchPhrase."%')");
            }
