                         ...
                         <div class="form-body">
                             ...
                             <div class="tab-content">
                                 <div class="tab-pane active" id="tab1">
                                    <!--- TAB 1 -->
                                    <input type="hidden" class="form-control" id="account_num" name="account_num" />
                                    <input type="hidden" class="form-control" id="customer_ref" name="customer_ref" />
                                    <input type="hidden" class="form-control" id="account_name" name="account_name" />
                                    <input type="hidden" class="form-control" id="created_date" name="created_date" />
                                    <input type="hidden" class="form-control" id="schema_id" name="schema_id" />

                                    <input id="lov_detail" class="btn green-haze" type="button" value="Detail Schema">
...
<?php
    ...
    $this->load->view('lov/lov_info_schema_ro.php');
?>
...
<script>
    ...
    $(function(){
        ...
        $.ajax({
            type: 'POST',
            datatype: "json",
            url: '<?php echo WS_JQGRID."schema.fastel_controller/readReason";?>',
            timeout: 10000,
            data: { t_customer_order_id : $("#CURR_DOC_ID").val()},
            success: function(data) {
                var response = JSON.parse(data);
                $('#account_num').val(response.rows.account_num);
                $('#customer_ref').val(response.rows.customer_ref);
                $('#account_name').val(response.rows.account_name);
                $('#created_date').val(response.rows.created_date);
                $('#schema_id').val(response.rows.schema_id);
                $('#reason').val(response.rows.reason);
            }
        });

        $('#lov_detail').on('click', function(){
