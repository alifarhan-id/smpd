...
<script>
    $(function($) {
        ...
        $("#grid-table").jqGrid({
            url: '<?php echo WS_JQGRID."transaksi_wf.t_vat_setllement_dtl_ro_otobuk_controller/read"; ?>',
            datatype: "json",
            ...
            colModel: [
                ...
                {label: 'Tanggal Transaksi', name: 'trans_date',  width: 180,align: "left"},
                {label: 'No.  Faktur',width: 150, align: "left",
                    formatter:function(cellvalue, options, rowObject) {

                        var bill_no = rowObject['bill_no'];
                        var bill_no_end = rowObject['bill_no_end'];
                        if (bill_no ==null){
                            bill_no="";
                        }
                        if (bill_no_end ==null){
                        ...
                        var faktur = bill_no+"-"+bill_no_end;
                        
                        return '<div>'+faktur+'</div>';

                    }
                },
                {label: 'Jumlah Faktur',name: 'bill_count',width: 150, align: "right"},
                {label: 'Omset',name: 'service_charge',width: 150, summaryTpl:"{0}",summaryType:"sum",formatter:'integer', formatoptions: {prefix:"", thousandsSeparator:',', defaultValue:'0'},align: "right"},                
                {label: 'Pajak',name: 'vat_charge',width: 150, summaryTpl:"{0}",summaryType:"sum",formatter:'integer', formatoptions: {prefix:"", thousandsSeparator:',', defaultValue:'0'},align: "right"},  
                {label: 'Keterangan',name: 'description',width: 180, align: "left"},                
            ],
            height: '100%',
            autowidth: true,
            viewrecords: true,
            ...
            gridComplete: function() {
                var $grid = $('#grid-table');
                var service_charge = $grid.jqGrid('getCol', 'service_charge', false, 'sum');
                var vat_charge = $grid.jqGrid('getCol', 'vat_charge', false, 'sum');
                $grid.jqGrid('footerData', 'set', { 'bill_count':'TOTAL',
            ...
            caption: "DETAIL SPTPD (Pelaporan Pajak)"
