...
class Cetak_formulir_restaurant_pdf extends CI_Controller{
	...
	function pageCetak() {
		...
		if ($t_customer_order_id != 0) {
			$sql .= "select * from f_cetak_pajak_restaurant(".$t_customer_order_id.", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "
		...
		$pdf->Ln();
		$pdf->Cell($lengthCell, $this->height, "WAJIB PAJAK RESTORAN", "LR", 0, 'C');
		$pdf->Ln();
		...
		*/
		...
		$pdf->Cell($per2, $this->height, "Pendataan Pajak Restoran", "TB", 0, 'L');
		$pdf->Cell($per3, $this->height, "", "TBR", 0, 'L');
		$pdf->Ln();
		...
		$pdf->Cell($kol6, $this->height*2, "", "TBLR", 0, 'C');
		$pdf->Ln($this->height-5);
		$pdf->Cell($kol1, $this->height*2, "", "L", 0, 'C');
		$pdf->Cell($kol2, $this->height*2, "Jenis Pelayanan", 0, 0, 'C');
		$pdf->Cell($kol3, $this->height*2, "Jumlah Kursi", 0, 0, 'C');
		$pdf->Cell($kol4, $this->height*2, "Jumlah Meja", 0, 0, 'C');
		$pdf->Cell($kol5, $this->height*2, "Daya Tampung", 0, 0, 'C');
		$pdf->Cell($kol6, $this->height+2, "Jumlah pengunjung", 0, 0, 'C');
		...
		$pdf->Cell($kol4, $this->height, "", 0, 0, 'C');
		$pdf->Cell($kol5, $this->height, "", 0, 0, 'C');
		$pdf->Cell($kol6, $this->height-2, "rata-rata per Bulan", 0, 0, 'C');
		...
		$pdf->RowMultiBorderWithHeight(array("",
											  $data2[$i]["service_type_desc"],
											  $data2[$i]["seat_qty"],
											  $data2[$i]["table_qty"],
											  $data2[$i]["max_service_qty"],
											  $data2[$i]["avg_subscription"],
											  "")
											 ,
										array('L',
										      'TBL',
