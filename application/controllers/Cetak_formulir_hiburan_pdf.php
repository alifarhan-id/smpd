...
class Cetak_formulir_hiburan_pdf extends CI_Controller{
	...
	function pageCetak() {
		...
		if ($t_customer_order_id != 0) {
			$sql .= "select * from f_cetak_pajak_entertaintment(".$t_customer_order_id.", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "
		...
		$pdf->Ln();
		$pdf->Cell($lengthCell, $this->height, "WAJIB PAJAK HIBURAN", "LR", 0, 'C');
		$pdf->Ln();
		...
		*/
		...
		$pdf->Cell($per2, $this->height, "Pendataan Pajak Hiburan", "TB", 0, 'L');
		$pdf->Cell($per3, $this->height, "", "TBR", 0, 'L');
		$pdf->Ln();
		//akhir tanda
		
		//kolom ke 7
		$kol1 = ($lengthCell * 1) / 50;
		$kol2 = ($lengthCell * 2) / 50;
		$kol3 = ($lengthCell * 9) / 50; 
		$kol4 = ($lengthCell * 6) / 50;
		$kol5 = ($lengthCell * 6) / 50; 
		$kol6 = ($lengthCell * 3) / 50;
		$kol7 = ($lengthCell * 6) / 50; 
		$kol8 = ($lengthCell * 4) / 50;
		$kol9 = ($lengthCell * 5) / 50; 
		$kol10 = ($lengthCell * 4) / 50;
		$kol11 = ($lengthCell * 3) / 50; 
		$kol12 = ($lengthCell * 1) / 50;
		...
		$pdf->Cell($kol8, $this->height, "", "TB", 0, 'C');
		$pdf->Cell($kol9, $this->height, "", "TB", 0, 'C');
		$pdf->Cell($kol10, $this->height, "", "TB", 0, 'C');
		$pdf->Cell($kol11, $this->height, "", "TB", 0, 'C');
		$pdf->Cell($kol12, $this->height, "", "RT", 0, 'C');
		$pdf->Ln();
		...
		$pdf->Cell($kol8, $this->height*2, "", 1, 0, 'C');
		$pdf->Cell($kol9, $this->height*2, "", 1, 0, 'C');
		$pdf->Cell($kol10, $this->height*2, "", 1, 0, 'C');
		$pdf->Cell($kol11, $this->height*2, "", 1, 0, 'C');
		$pdf->Cell($kol12, $this->height*2, "", "R", 0, 'C');
		$pdf->Ln($this->height-5);
		$pdf->Cell($kol1, $this->height*2, "", "L", 0, 'C');
		$pdf->Cell($kol2, $this->height*2, "No", 0, 0, 'C');
		$pdf->Cell($kol3, $this->height*2, "Jenis Hiburan", 0, 0, 'C');
		$pdf->Cell($kol4, $this->height+2, "Cover Charge/", 0, 0, 'C');
		$pdf->Cell($kol5, $this->height+2, "Jumlah Lembar", 0, 0, 'C');
		$pdf->Cell($kol6, $this->height+2, "Jumlah", 0, 0, 'C');
		$pdf->Cell($kol7, $this->height+2, "Jumlah PL", 0, 0, 'C');
		$pdf->Cell($kol8, $this->height+2, "Boking/", 0, 0, 'C');
		$pdf->Cell($kol9, $this->height*2, "F & B", 0, 0, 'C');
		$pdf->Cell($kol10, $this->height+2, "Porsi/", 0, 0, 'C');
		$pdf->Cell($kol11, $this->height+2, "Lain-", 0, 0, 'C');
		$pdf->Cell($kol12, $this->height*2, "", "R", 0, 'C');
		$pdf->Ln($this->height);
		$pdf->Cell($kol1, $this->height, "", "L", 0, 'C');
		$pdf->Cell($kol2, $this->height, "", 0, 0, 'C');
		$pdf->Cell($kol3, $this->height, "", 0, 0, 'C');
		$pdf->Cell($kol4, $this->height-2, "HTM/Tarif", 0, 0, 'C');
		$pdf->Cell($kol5, $this->height-2, "Meja/Kursi", 0, 0, 'C');
		$pdf->Cell($kol6, $this->height-2, "Room", 0, 0, 'C');
		$pdf->Cell($kol7, $this->height-2, "Pramuria/Pemijat", 0, 0, 'C');
		$pdf->Cell($kol8, $this->height-2, "Jam", 0, 0, 'C');
		$pdf->Cell($kol9, $this->height, "", 0, 0, 'C');
		$pdf->Cell($kol10, $this->height-2, "Orang", 0, 0, 'C');
		$pdf->Cell($kol11, $this->height-2, "lain", 0, 0, 'C');
		$pdf->Cell($kol12, $this->height, "", "R", 0, 'C');
		$pdf->Ln();
		
		//isi kolom
		$no = 1;
		$pdf->SetWidths(array($kol1, $kol2, $kol3, $kol4, $kol5, $kol6, $kol7, $kol8, $kol9, $kol10, $kol11, $kol12));
		$pdf->SetAligns(array("C", "C", "L", "R", "C", "C", "C", "C", "C", "C", "C", "C"));
		for ($i=0; $i<count($data2); $i++) {
		$pdf->RowMultiBorderWithHeight(array("",
											  $no,
											  $data2[$i]["entertainment_desc"],
											  $data2[$i]["service_charge_wd"],
											  $data2[$i]["seat_qty"],
											  $data2[$i]["room_qty"],
											  $data2[$i]["clerk_qty"],
											  $data2[$i]["booking_hour"],
											  $data2[$i]["f_and_b"],
											  $data2[$i]["portion_person"],
											  "",
											  "")
											 ,
										array('L',
		...
		$pdf->Cell($kol8, $this->height, "", "B", 0, 'C');
		$pdf->Cell($kol9, $this->height, "", "B", 0, 'C');
		$pdf->Cell($kol10, $this->height, "", "B", 0, 'C');
		$pdf->Cell($kol11, $this->height, "", "B", 0, 'C');
		$pdf->Cell($kol12, $this->height, "", "RB", 0, 'C');
		$pdf->Ln();
