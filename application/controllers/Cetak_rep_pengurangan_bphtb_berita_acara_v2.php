...
class Cetak_rep_pengurangan_bphtb_berita_acara_v2 extends CI_Controller{
	var $fontSize = 10;
	var $fontFam = 'Arial';
	var $yearId = 0;
	var $yearCode="";
	...
	function pageCetak(){
		...
		$data = $this->getBeritaAcaraV2($t_bphtb_registration_id);
		...
		if($t_bphtb_registration_id != 0){
			...
			$pdf->SetFont("Arial", "B", 8);
			$pdf->Cell($lbody1, $this->height, "", "", 0, "");
			$pdf->Cell($lbody4+10, $this->height, "Akta/ Risalah Lelang/ Keputusan", "", 0, "");
			$pdf->SetFont("Arial", "", 8);
			$pdf->Cell($lbody15-10, $this->height, ": ".$data['keterangan_opsi_c'], "", 0, "");
			$pdf->Ln();
			$pdf->SetFont("Arial", "B", 8);
			$pdf->Cell($lbody1, $this->height, "", "", 0, "");
			$pdf->Cell($lbody4+10, $this->height, "Pemberian Hak/ Putusan Hakim/", "", 0, "");
			$pdf->Ln();
			$pdf->Cell($lbody1, $this->height, "", "", 0, "");
			$pdf->Cell($lbody4+10, $this->height, "Dokumen lainnya", "", 0, "");
			$pdf->Ln();
			$this->barisBaru($pdf,"", "- Nomor", ": " . $data['nomor_notaris']);
			$this->barisBaru($pdf,"", "- Tanggal", ": " . $this->beautyDate($data['tanggal_sk']));
			$pdf->SetFont("Arial", "B", 8);
			$pdf->Cell($lbody1, $this->height, "", "", 0, "");
			$pdf->Cell($lbody4+10, $this->height, "NJOP", "", 0, "");
			$pdf->SetFont("Arial", "", 8);
			$pdf->Cell($lbody15-10, $this->height, ": ". "Rp.".number_format($data['npop'], 0, ",", "."), "", 0, "");
			$pdf->Ln();
			$pdf->Ln();
			...
			$pdf->Ln();
			$this->barisBaru2($pdf,$lbody1+$lbody1, "- Besaran Pengenaan Pengurangan ", "50%", "Rp", $data["bphtb_discount"]);
	...
	function barisBaru_bold($pdf,$section, $field, $data){
		$pdf->SetFont("Arial", "B", 8);
		$lbody = $this->lengthCell / 20;
		$lbody1 = $lbody * 1;
		$lbody4 = $lbody * 4;
	...
	function getBeritaAcaraV2($t_bphtb_registration_id){
		$whereClause='';
		$data = array();
