...
class Cetak_formulir_stpd_pdf extends CI_Controller{
	...
	function pageCetak() {
		...
		$this->tulis("2. Apabila STPD ini tidak atau kurang dibayar setelah lewat waktu paling lama 20 hari kalender sejak STPD ini", "L", $pdf);
		...
		$pdf->Cell($lbody3 - 25, $this->height, "", "L", 0, 'L');
		$pdf->Cell($lbody1 + 25, $this->height, "a.n.KEPALA BADAN PENDAPATAN DAERAH", "R", 0, 'C');
		$pdf->Ln();
		$pdf->Cell($lbody3 - 25, $this->height, "", "L", 0, 'L');
		$pdf->Cell($lbody1 + 25, $this->height, "Kepala Bidang Pendapatan", "R", 0, 'C');
		$this->newLine($pdf);
		$pdf->Cell($this->lengthCell, $this->height, "", "LR", 0, 'L');
		$pdf->Ln();

		$pdf->Cell($lbody3 - 25, $this->height, "", "L", 0, 'L');
		$pdf->Cell($lbody1 + 25, $this->height, getValByCode('KABID_PENDAPATAN'), "R", 0, 'C');
		$pdf->Ln();

		$pdf->Cell($lbody3 - 22, $this->height, "", "L", 0, 'L');
		$pdf->Cell($lbody1 + 20, $this->height, "NIP. ".getValByCode('NIP_KABID_PENDAPATAN'), "T", 0, 'C'); //isi nip
