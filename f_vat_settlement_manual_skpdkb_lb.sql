...
CREATE OR REPLACE FUNCTION sikp.f_vat_settlement_manual_skpdkb_lb(
	...
	IF ada > 0 THEN		
     ...
     ldt_due_date := add_months(v_start_date,1) + ln_due_in_day ;
	   ...
	   UPDATE t_vat_setllement SET
		  ...
		  total_vat_amount = nvl(temp_payment_vat_amount,0) - nvl(v_vat_amount,0) ,
...
when others then
	...
	if o_mess is null then
           o_mess := to_char(nvl(ln_result_code,-99))||' : '|| nvl(ls_result_msg,' Pembuatan SKPDKB Lebih Bayar gagal .') ;
	end if;
	--o_mess := 'sdsd';
	o_cust_order_id := 0;
	--rollback;
